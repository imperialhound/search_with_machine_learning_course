...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # generate response of feature logs 
        response = self.opensearch.search(body=log_query, index=self.index_name)
        
        # Iterate through results and append to dataframe
        feature_dataframe = pd.DataFrame()

        for individual_feature in response["hits"]["hits"]:
            feature_set = {rank_feature.get("name"): rank_feature.get("value", 0)
            for rank_feature in individual_feature["fields"]["_ltrlog"][0]["log_entry"]}
            feature_set["doc_id"] = individual_feature["_id"]
            feature_set["sku"] = individual_feature["_source"]["sku"][0]
            feature_set["query_id"] = query_id
            feature_dataframe = feature_dataframe.append(feature_set, ignore_index=True)

        return feature_dataframe.astype({'doc_id': 'int', 'query_id': 'int', 'sku': 'int'})
        # IMPLEMENT_END
    ...
    def __num_clicks(self, all_skus_for_query, test_sku):
        click_numes = all_skus_for_query[all_skus_for_query == test_sku].count()
        return  click_numes
